...
from keyboards import kb_client, kb_vote, kb_murder, kb_skip 
...
async def get_result_game(message: types.Message, state=FSMContext): 
    ...
    async with state.proxy() as data:
        ...
        answer+= '\nПроверки дона:'
        for i, night in enumerate(data['nights']):
            answer += f"{night['don_check'] if night['don_check'] is not None else ''} " 
        answer+= '\nПроверки шерифа:'
        for i, night in enumerate(data['nights']):
            answer += f" {night['sheriff_check'] if night['sheriff_check'] is not None else ''}"
        answer+= f"\nЛХ: {data['best_move']}"
...
async def get_nick(message: types.Message, state=FSMContext):
    ...
    else: await message.answer(Answer.NICK_ERROR.value)
...
async def get_nominations(message: types.Message, state=FSMContext):
    ...
    async with state.proxy() as data:
        if not data.get('days'): data['days'] = []
...
async def get_murder(message: types.Message, state=FSMContext):
    ...
    async with state.proxy() as data:
        if not data.get('nights'): data['nights'] = []
...
def register_handlers_client(dp : Dispatcher):
    dp.register_message_handler(get_result_game, state='*', commands=['Закончить'])
